...
namespace aoc2020.Puzzles.Solutions
{
    ...
    public sealed class Day23 : SolutionBase
    {
        ...
        private static IEnumerable<(int Address, long X, long Y)> HandleOutgoingPackets(Day11.SynchronousIntMachine machine, Queue<long>[] queues)
        {
            while (machine.RunUntilBlockOrComplete() == Day11.SynchronousIntMachine.ReturnCode.WrittenOutput)
            {
        ...
        private static void HandleIncomingPackets(Day11.SynchronousIntMachine[] machines, Queue<long>[] queues)
        {
        ...
        private static (int Address, long X, long Y) ReadPacket(Day11.SynchronousIntMachine machine)
        {
        ...
        private static void SetupMachines(string input, out Day11.SynchronousIntMachine[] machines, out Queue<long>[] queues)
        {
            var memory = Day09.IntMachineBase.ParseProgram(input);
            machines = Enumerable.Range(0, MachineCount).Select(_ => new Day11.SynchronousIntMachine(memory.ToArray())).ToArray();
