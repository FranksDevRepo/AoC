...
using IntMachineBase = aoc2020.Puzzles.Solutions.Day09.IntMachineBase;
using Point = aoc2020.Puzzles.Solutions.Day10.Point;
...
namespace aoc2020.Puzzles.Solutions
{
    ...
    public sealed class Day11 : SolutionBase
    {
        public override async Task<string> Part1Async(string input)
        {
            ...
            var canvas = new Dictionary<Day10.Point, int>();
        ...
        {
            var intMachine = new SynchronousIntMachine(input);
            var canvas = new Dictionary<Day10.Point, int> { [new Day10.Point(0, 0)] = 1 };
            await Paint(intMachine, canvas);

            return Render(canvas);
        }

        private static string Render(Dictionary<Day10.Point, int> canvas)
        {
            var whitePoints = canvas.Where(x => x.Value == 1).Select(x => x.Key).ToList();
            var topLeft = new Day10.Point(whitePoints.Min(x => x.X), whitePoints.Min(x => x.Y));
            var bottomRight = new Day10.Point(whitePoints.Max(x => x.X), whitePoints.Max(x => x.Y));
            var resultSb = new StringBuilder();
            for (var x = topLeft.X; x <= bottomRight.X; x++)
            {
                for (var y = topLeft.Y; y <= bottomRight.Y; y++)
                {
                    if (canvas.TryGetValue(new Day10.Point(x, y), out var color))
                    {
        ...
        private async Task<int> Paint(SynchronousIntMachine intMachine, Dictionary<Day10.Point, int> canvas)
        {
            var paintedPositions = new HashSet<Day10.Point>();
            var directions = new[] { new Day10.Point(-1, 0), new Day10.Point(0, 1), new Day10.Point(1, 0), new Day10.Point(0, -1) };
            var direction = 0;
            var pos = new Day10.Point(0, 0);
        ...
        public sealed class SynchronousIntMachine : Day09.IntMachineBase
        {
