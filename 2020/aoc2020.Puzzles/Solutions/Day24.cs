...
namespace aoc2020.Puzzles.Solutions
{
    ...
    public sealed class Day24 : SolutionBase
    {
        ...
        private int[] ParseMap(string input)
        {
            ...
            myMiddlePoint = new Day10.Point(myWidth / 2, myHeight / 2);
            myMiddlePos = GetPos(myMiddlePoint);

            return map;
        }

        private int GetPos(Day10.Point point) => GetPos(point.X, point.Y);
        private int GetPos(int x, int y) => x + y * myWidth;
        private Day10.Point GetPoint(int pos) => new Day10.Point(pos % myWidth, pos / myWidth);
        private bool IsWithinBounds(Day10.Point p) => p.X >= 0 && p.X < myWidth && p.Y >= 0 && p.Y < myHeight;

        private int myWidth;
        private int myHeight;
        private int myMapSize;
        private Day10.Point myMiddlePoint;
        private int myMiddlePos;
        private readonly Day10.Point[] myDirections = new[] { new Day10.Point(0, -1), new Day10.Point(1, 0), new Day10.Point(0, 1), new Day10.Point(-1, 0) };
    }
}
